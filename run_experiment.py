...
def show_available_models(model_configs):
    ...
    console.print("\n[bold cyan]Available model[/bold cyan]\n")
...
def run_training(model_name, model_configs, X_train_tfidf, y_train, X_test_tfidf, y_test, tracker, scoring, random_seed,run_test=True, save_best=True):
    ...
    search_result = tuner.fit(X_train_tfidf, y_train, save_best=save_best)
...
def show_saved_model(model_dir):
    ...
    if not model_paths:
        console.print("[yellow]No saved .pkl model found in outputs/models.[/yellow]")
...
def test_flow(model_dir, X_test_tfidf, y_test, tracker):
    console.clear()
    print_logo()
...
def main():
    ...
    while True:
        ...
        if action == "Train / tune a model":
            ...
            test_flow(model_dir, X_test_tfidf, y_test, tracker)
